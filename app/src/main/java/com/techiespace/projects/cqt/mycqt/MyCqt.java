package com.techiespace.projects.cqt.mycqt;
...
public class MyCqt {
    ...
    public MyCqt(float sampleRate, float minFreq, float maxFreq,float binsPerOctave) {
        this(sampleRate,minFreq,maxFreq,binsPerOctave,0.001f,1.0f);
    }


    public MyCqt(float sampleRate, float minFreq, float maxFreq,float binsPerOctave, float threshold,float spread) {
        ...
        int numberOfBins = 88*3;//(int) Math.ceil(binsPerOctave * Math.log(maximumFreqency / minimumFrequency) / Math.log(2));
    ...
    public void calculate(float[] inputBuffer) {
        ...
        for (int i = 0; i < qKernel.length; i++) {
            ...
            float t_r = 0;
            float t_i = 0;
            for (int j = 0, l = 0; j < kernel.length-2; j += 2, l++) {
                int jj = indexes[l];
